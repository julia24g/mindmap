...
describe('DashboardPage', () => {
  ...
  it('shows error state when there is an error', () => {
    ...
    render(<DashboardPage />);
    expect(screen.getByText('Error loading graph: Failed to fetch graph')).toBeInTheDocument();
  });
  ...
  it('shows empty state when there is no graph data', () => {
    ...
    expect(screen.getByText("You haven't added any content yet. Get started by adding your first piece of content above.")).toBeInTheDocument();
  });
  ...
  it('does not show empty state when there is graph data', () => {
    ...
    const mockGraph = {
      nodes: [
        { id: '1', label: 'content', name: 'Node 1' },
      ],
      edges: [],
    };
